-- ═══════════════════════════════════════════════════════════════════════
-- RESET + APPLY — for the new Supabase project psnrabnwlqqlwezjfunp.
--
-- Use this single file if a previous partial migration left the schema
-- in a half-built state (e.g. the failure "column seller_id does not
-- exist" because tables existed without their seller_id column).
--
-- It is SAFE to run on the new project because nothing belonging to
-- leadecombot is there yet. DO NOT run this on a production project
-- with data — it drops every leadecombot table cascade.
-- ═══════════════════════════════════════════════════════════════════════

-- ── 1. Drop everything leadecombot-owned, cascade ──────────────────────
drop table if exists public.orders                       cascade;
drop table if exists public.messages                     cascade;
drop table if exists public.customer_conversations       cascade;
drop table if exists public.product_countries            cascade;
drop table if exists public.products                     cascade;
drop table if exists public.seller_whatsapp_sessions     cascade;
drop table if exists public.app_users                    cascade;
drop table if exists public.sellers                      cascade;

drop function if exists public.current_seller_id()                              cascade;
drop function if exists public.current_is_admin()                               cascade;
drop function if exists public.touch_updated_at()                               cascade;
drop function if exists public.create_seller_for_self(text, text, text[], text) cascade;
