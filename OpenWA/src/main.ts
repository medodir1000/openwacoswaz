...
// ── Crash resilience (multi-tenant safety) ───────────────────────────────
// A single WhatsApp session misbehaving must NEVER take down the whole
// gateway — every seller shares this one process. whatsapp-web.js can throw
// asynchronously during session teardown; the worst offender on Windows is
// LocalAuth.logout() doing an rmdir on a session folder Chromium still has
// locked (ENOTEMPTY), which bubbles up as an uncaughtException and exits the
// process (observed: it killed :2785 and every dashboard call started
// returning 502 Bad Gateway). Swallow these so the gateway keeps serving;
// the affected session just reconnects on its own schedule.
process.on('uncaughtException', err => {
  console.error('[uncaughtException] gateway kept alive:', (err && err.stack) || err);
});
process.on('unhandledRejection', reason => {
  console.error('[unhandledRejection] gateway kept alive:', reason);
});
