...
export class WhatsAppWebJsAdapter extends EventEmitter implements IWhatsAppEngine {
  ...
  // Show/hide the "typing..." indicator in WhatsApp. Called by
  // MessageService when typingMs > 0 on a send-text request. Wrapped in
  // try/catch because chat fetches can fail right after disconnects and
  // we'd rather still send the message than blow up the whole call.
  ...
  // Mark inbound messages in this chat as read (blue ticks). Used as the
  // first touch in our human-presence sequence so customers see "seen"
  // before "typing..."  appears.
