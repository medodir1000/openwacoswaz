...
@Module({
  imports: [
    ...
    // Rate limiting
    //
    // The default values (10 req/s, 100 req/min, 1000 req/hr) were
    // built for an external public API where each tenant might issue a
    // handful of REST calls per minute. In *this* deployment OpenWA is
    // a trusted internal gateway with three persistent clients hitting
    // it:
    //   1. The leadecombot brain (/wa/status polling + send-text per
    //      reply + webhook auto-register on boot)
    //   2. The OpenWA dashboard's React app (session badge + message
    //      list refresh every ~2s while a tab is open)
    //   3. Hot-reload tooling (vite/nest each ping on startup)
    //
    // Together they easily clear 10 req/s during normal use, which
    // jams the throttler and makes EVERY subsequent request — including
    // the brain's send-text replies — return 429. Customer-visible
    // symptom: bot stops replying entirely until the throttler window
    // expires. Bumping these to dev-friendly numbers fixes that.
    // Defaults can still be overridden via env (api.rateLimit.*) for
    // public deployments where the tighter limits actually matter.
        ...
        throttlers: [
          {
            ...
            limit: configService.get<number>('api.rateLimit.shortLimit', 200),
          },
          {
            name: 'medium',
            ttl: configService.get<number>('api.rateLimit.mediumTtl', 60000),
            limit: configService.get<number>('api.rateLimit.mediumLimit', 3000),
          },
          {
            name: 'long',
            ttl: configService.get<number>('api.rateLimit.longTtl', 3600000),
            limit: configService.get<number>('api.rateLimit.longLimit', 50000),
          },
        ],
      }),
    }),
