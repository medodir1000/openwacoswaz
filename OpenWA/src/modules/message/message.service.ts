...
export class MessageService {
  ...
  async sendText(sessionId: string, dto: SendTextMessageDto): Promise<MessageResponseDto> {
    ...
    try {
      // Human-like presence: mark seen (blue ticks) then show typing for
      // a believable composition window before the message lands.
      // Defaults to off so other callers (Message Tester, bulk send,
      // plugins) keep the same instant-send behaviour.
      if (finalDto.markSeen && engine.markChatRead) {
        try { await engine.markChatRead(finalDto.chatId); } catch { /* best-effort */ }
      }
      const typingMs = Math.max(0, Math.min(30_000, finalDto.typingMs ?? 0));
      if (typingMs > 0 && engine.setTyping) {
        try { await engine.setTyping(finalDto.chatId, true); } catch { /* best-effort */ }
        await new Promise((r) => setTimeout(r, typingMs));
        try { await engine.setTyping(finalDto.chatId, false); } catch { /* best-effort */ }
      }
